...
- minimal data frame (converted by the package into payload; `response` and `current_query_status` can be supplied)
...
For data-frame imports via `dq_import()`, `current_query_status` is optional and may be one of
`OPEN`, `CLOSED`, `VERIFIED`, `DEVERIFIED`, or blank (default is `OPEN`).
...
Users who require repeat-instrument–aware query creation must modify the upstream
