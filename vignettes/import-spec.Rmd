...
1. Which fields are needed to create a new status row.
2. Which fields are needed to add a comment/resolution to an existing status.
...
- Import only adds new resolutions (comment/reply rows); it does not modify existing rows.
- Deduplication is based on `ts + user` (resolved to `user_id`) within a status; duplicates are skipped.
- `data` must be provided as JSON text.

## Top-level payload structure

`data` must represent a collection of status rows. Two JSON shapes are accepted:
...
## Fields required to create a new status row

When no existing status is matched, the API attempts to create a new status row. To succeed,
...
## Fields required to add comment/resolution to an existing status
...
Status-related fields (recommended):
...
For existing statuses, the most stable approach is to set the same existing status ID in both
`dataRow$status_id` and `resolution$status_id`.

## Minimal payload examples

### A. Create a new status + one comment
...
### B. Add one comment to an existing status
...
- If import fails, first verify required field completeness and `project_id / status_id` alignment.
...
Although `repeat_instrument` is used when attempting to match existing status records,
it is not written during status creation. As a result:

- A new status cannot be created that is scoped specifically to a repeating instrument.
- Statuses created via the API will have `repeat_instrument = NULL`.
...
Users who require repeat-instrument–aware status creation must modify the upstream
